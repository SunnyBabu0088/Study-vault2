...
export default function AiHubView({ onClose }) {
  ...
  const handleSendMessage = async (customText) => {
    ...
    try {
      ...
      console.error('[AI Hub Request Error]', err);
  ...
  return (
    <div className="fixed inset-0 z-[999999] h-[100dvh] w-[100vw] bg-[var(--background)] text-[var(--text-primary)] flex flex-col overflow-hidden select-none animate-in fade-in zoom-in-98 duration-300">
      {/* HIDDEN FILE INPUT */}
      <input ref={fileInputRef} type="file" onChange={handleFileSelect} className="hidden" />

      {/* MINIMAL AI HUB HEADER: ← Back | StudyVault AI Hub | ◷ History | × Close */}
      <header className="flex items-center justify-between px-4 py-3 border-b border-[var(--border)] bg-[var(--surface)] shrink-0">
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={onClose}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[var(--surface-secondary)] hover:bg-[var(--border)]/40 text-xs font-bold text-[var(--text-primary)] cursor-pointer transition-colors"
          >
          ...
          <div className="flex items-center gap-2">
            <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-gradient-to-tr from-[#3b52cf] to-[#7c3aed] text-white shadow-md">
              <Sparkles className="h-4 w-4 animate-pulse" />
            </div>
            <h2 className="text-sm font-bold m-0 text-[var(--text-primary)]">
              StudyVault AI Hub
            </h2>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`flex items-center justify-center h-8 w-8 rounded-xl border text-xs font-bold transition-all cursor-pointer ${
          ...
          <button
            type="button"
            onClick={onClose}
            className="grid h-8 w-8 place-items-center rounded-xl bg-[var(--surface-secondary)] border border-[var(--border)] text-[var(--text-muted)] hover:text-[var(--text-primary)] cursor-pointer"
            title="Close AI Hub"
          >
            <X className="h-4 w-4" />
          </button>
      ...
      <div className="relative flex-1 overflow-hidden flex">
        ...
        <div className="flex-1 overflow-y-auto p-4 pb-32 space-y-6 max-w-3xl mx-auto w-full">
          {messages.length === 0 ? (
            /* CLEAN EMPTY STATE */
            <div className="h-full flex flex-col items-center justify-center py-20 text-center space-y-4 animate-in fade-in zoom-in-98 duration-300 max-w-md mx-auto px-4">
              ...
              <div className="space-y-2">
                <h3 className="text-2xl font-bold text-[var(--text-primary)] m-0">StudyVault AI</h3>
                <p className="text-base font-semibold text-[var(--text-primary)] m-0">How can I help you today?</p>
                <p className="text-xs text-[var(--text-muted)] leading-relaxed m-0">
                  Ask anything about coding, quizzes, images, study materials, writing, or anything else.
                </p>
              </div>
            </div>
          ) : (
                    ...
                    <div
                      className={`p-4 rounded-3xl text-xs leading-relaxed ${
                        isUser
                          ...
                          : 'bg-[var(--surface-secondary)] border border-[var(--border)] text-[var(--text-primary)] rounded-tl-sm shadow-sm'
                      }`}
                    >
                      ...
                      {meta.type === 'quiz' && meta.questions && (
                        <div className="mt-4 pt-3 border-t border-[var(--border)]/50 space-y-4">
                          {meta.questions.map((q) => {
                            ...
                            return (
                              <div key={q.id} className="p-3.5 rounded-2xl bg-[var(--surface)] border border-[var(--border)] space-y-2 text-left">
                                <p className="font-bold text-xs text-[var(--text-primary)] m-0">Q{q.id}. {q.question}</p>
                                <div className="space-y-1.5 pt-1">
                                  {q.options.map((opt, oIdx) => {
                                    let btnClass = "bg-[var(--surface-secondary)] text-[var(--text-secondary)] hover:bg-[var(--border)]/40";
                                    if (isAnswered) {
          ...
          {loading && (
            <div className="flex items-center gap-3 animate-in fade-in duration-200">
              ...
              <div className="px-4 py-3 rounded-2xl bg-[var(--surface-secondary)] border border-[var(--border)] text-xs text-[var(--text-muted)] flex items-center gap-2">
      ...
      <footer className="p-4 border-t border-[var(--border)]/60 bg-[var(--surface)]/80 backdrop-blur-md shrink-0 pb-[max(1rem,env(safe-area-inset-bottom))]">
